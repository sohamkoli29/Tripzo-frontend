...
export default function PaymentsPage() {
  ...
  const filteredPayments = payments.filter((p) => {
    if (filter === "paid")
      return p.status === "completed" || p.status === "cash";
        ...
        {[
          ...
          },
          {
            ...
            value: payments.filter(
              (p) => p.status === "completed" || p.status === "cash",
            ).length,
      ...
      {filteredPayments.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-gray-500 text-sm mt-2">
            {filter === "all"
              ? "Complete a ride to see payments here"
              : "No " + filter + " payments"}
          </p>
        </div>
      ) : (
          ...
          <div className="divide-y divide-gray-800">
            {filteredPayments.map((payment) => {
              ...
              return (
                <div
                  key={payment.id}
                  className="p-5 flex items-center justify-between gap-4 hover:bg-gray-800/40 transition"
                >
                  {/* Left */}
